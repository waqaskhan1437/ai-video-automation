    ...
    while (attempts < maxAttempts) {
      ...
      if (status.status === 'completed') {
        if (status.downloads && status.downloads.length > 0) {
          ...
          console.log(`Saved clip ${index + 1}: ${filename}`);
...
async function generateClips() {
  ...
  console.log(`Clips: ${NUM_CLIPS} x ${CLIP_DURATION}s = ${NUM_CLIPS * CLIP_DURATION}s total`);
  console.log('='.repeat(50) + '\n');
  
  if (!API_KEY) {
    console.log('WARNING: No Magic Hour API key found!');
    console.log('Set MAGIC_HOUR_API_KEY in GitHub Secrets');
    console.log('Demo mode: Creating placeholder files\n');
    ...
    for (let i = 0; i < NUM_CLIPS; i++) {
      ...
      const demoContent = Buffer.from(JSON.stringify({
        index: i + 1,
    ...
    console.log('\n' + '='.repeat(50));
    console.log(`Demo clips created!`);
    console.log('='.repeat(50));
    console.log('\nSETUP REQUIRED:');
